...
function backup_clashing_targets(){
  ...
  while IFS= read -r -d '' entry; do
    source_list+=("$(basename "$entry")")
  done < <(find "$source_dir" -maxdepth 1 -mindepth 1 -print0)
  while IFS= read -r -d '' entry; do
    target_list+=("$(basename "$entry")")
...
function resolve_pkg_name(){
  ...
  case "$OS_GROUP_ID" in
    "fedora")
      case "$cmd" in ip) echo "iproute";; *) echo "$cmd";; esac ;;
    *)
      case "$cmd" in ip) echo "iproute2";; *) echo "$cmd";; esac ;;
  esac
}
